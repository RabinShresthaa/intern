...
export default function UserReportsPage() {
  ...
  const getViewUrl     = (fileId) => `http://localhost:4000/api/reports/file/${fileId}`;
  const getDownloadUrl = (fileId) => `http://localhost:4000/api/reports/file/${fileId}?download=true`;
          ...
          {reports.map((item) => (
            <div key={item._id} className={styles.reportCard}>
              <div className={styles.cardHeader}>
                <h3>{item.title}</h3>
                <p className={styles.description} style={{ overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', wordBreak: 'break-word' }}>{item.description}</p>
              </div>
      ...
      {viewingFile && (
        <div className={styles.modalOverlay} onClick={() => setViewingFile(null)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setViewingFile(null)} className={styles.closeBtn}>
              &times;
            </button>
            ...
            <div className={styles.previewContainer}>
              ...
              {viewingFile.category === "video" && (
                <video key={viewingFile.url} controls autoPlay className={styles.videoPlayer}>
