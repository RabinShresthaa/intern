...
<body>${rawHtml}</body>
            ...
            return res.status(500).json({ message: `XLSX conversion failed: ${err.message}` });
        }
    }
...
<body>${result.value}</body>
            ...
            return res.status(500).json({ message: `DOCX conversion failed: ${err.message}` });
        }
    }
    ...
    if (ext === '.heic' || ext === '.heif') {
        try {
            ...
            return res.status(500).json({ message: 'HEIC conversion failed. Try downloading the file.' });
        }
    }
...
router.delete('/:id/file/:fileId', async (req, res) => {
    try {
        ...
        const filePath = path.join(__dirname, '../public/uploads', req.params.fileId);
        if (fs.existsSync(filePath)) {
